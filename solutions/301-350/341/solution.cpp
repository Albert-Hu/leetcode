...
class NestedInteger {
public:
  NestedInteger(int val) : m_isInteger(true), m_value(val) {}

  NestedInteger(vector<NestedInteger> list) : m_isInteger(false), m_value(0) {
    m_list = list;
  }
...
public:
  NestedIterator(vector<NestedInteger> &nestedList) {
    function<void(const vector<NestedInteger> &)> unpack =
        [&](const vector<NestedInteger> &l) {
          for (auto element : l) {
